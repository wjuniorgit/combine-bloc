...
open class Bloc<Event: Equatable, State: Equatable> {
  ...
  public init(
    initialValue: State,
    mapEventToState: @escaping (Event, State, @escaping (State) -> Void)
      -> Void
  ) {
    value = initialValue
  ...
  public var subscriber: BlocSubscriber<Event> {
    if _subscriber == nil {
      _subscriber = BlocSubscriber<Event>(
        ...
        onReceiveInputSubscription: { subscription in
          self.eventSubject.send(subscription: subscription)
        }
      )
    }
    return _subscriber!
  ...
  private let mapEventToState: (Event, State, @escaping (State) -> Void)
    -> Void
  private var _subscriber: BlocSubscriber<Event>?
  private let eventSubject = PassthroughSubject<Event, Never>()
  private let stateSubject = PassthroughSubject<State, Never>()
  internal let transitionSubject = PassthroughSubject<
...
public final class BlocSubscriber<Input>: Subscriber {
  fileprivate init(
    onReceiveInput: @escaping (Input) -> Void,
    onReceiveInputSubscription: @escaping (Subscription) -> Void
  ) {
...
public extension Bloc {
  ...
  func debug(
    ...
    onTransition: ((Transition<Event, State>) -> Void)? = nil,
    onInit: (() -> Void)? = nil,
    onDeinit: (() -> Void)? = nil
  ) -> Bloc<Event, State> {
