...
final class FilteredTodosBloc: Bloc<FilteredTodosEvent, FilteredTodosState> {
  ...
  init(sortedTodosBloc: Bloc<SortedTodosEvent, SortedTodosState>) {
    ...
    super
      .init(initialValue: FilteredTodosState(
        todosState: .Loading,
        filterRule: .none
      )) { event, state, emit in
        ...
        case let .TodosStateUpdated(todoState):
          switch todoState {
          case .Error, .Loading:
            emit(FilteredTodosState(
              todosState: todoState,
              filterRule: state.filterRule
            ))
          case let .Loaded(todos):
            emit(FilteredTodosState(
              todosState: .Loaded(
                FilteredTodosBloc
                  .filter(todos, currentFilterRule)
              ),
              filterRule: state.filterRule
            ))
          }
        ...
        if currentFilterRule != state.filterRule {
          if case let TodosState.Loaded(todos) = sortedTodosBloc.value
            .todosState
          {
            emit(FilteredTodosState(
              todosState: .Loaded(
    ...
    cancellable = sortedTodosBloc.publisher.sink { sortedTodosState in
      self.send(.TodosStateUpdated(sortedTodosState.todosState))
    }
  }
